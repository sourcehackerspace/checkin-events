		...
		<div class="card-content">
			<form action="{{ route('crud.events.storage') }}" method="POST" enctype="multipart/form-data">
				...
				<div class="input-field">
					<input type="text" name="address" class="validate{{ $errors->has('address') ? ' invalid' : '' }}" id="addressInput" value="{{ old('address') }}">
				...
				<div class="row">
					<div class="input-field col l4">
						<input type="date" id="dateInput" name="date" class="datepicker {{ $errors->has('date') ? 'validate invalid' : '' }}" data-value="{{ old('date') }}">
						@if ($errors->has('date'))
					...
					<div class="input-field col l4">
						<input type="time" name="time" class="{{ $errors->has('time') ? 'validate invalid' : '' }}" value="{{ old('time') }}" id="timeInput">
						@if ($errors->has('time'))
					...
					<div class="input-field col l4">
						<input type="number" name="quota" class="validate{{ $errors->has('qouta') ? ' invalid' : '' }}" id="qoutaInput" value="{{ old('quota') }}" min="1">
						@if ($errors->has('quota'))
				...
				<div class="row">
					<div class="col l6">
						<p>
							<input type="checkbox" id="cost" name="hascost"/>
				...
				<br>
				<div class="row">
					<button type="submit" class="btn accent waves-effect waves-light col s12 l4 offset-l4">crear evento</button>
				</div>
			</form>
		</div>
	</div>
